...
(() => {
  ...
  const alternateShuffleInput = document.getElementById('ig-alt-shuffle');
  ...
  function syncAlternateUi(mainEnabled = !!source) {
    ...
    alternateShuffleInput.disabled = !available;
  ...
  function makeTileShuffle(random, tileCount) {
    const map = new Uint32Array(tileCount);
    for (let tileIndex = 0; tileIndex < tileCount; tileIndex++) map[tileIndex] = tileIndex;
    for (let tileIndex = tileCount - 1; tileIndex > 0; tileIndex--) {
      const swapIndex = Math.floor(random() * (tileIndex + 1));
      const value = map[tileIndex];
      map[tileIndex] = map[swapIndex];
      map[swapIndex] = value;
    }
    return map;
  }
  ...
  function makeFaultPlan(frameIndex) {
    ...
    const shuffleRandom = mulberry32(
      (seed ^ Math.imul(frameIndex + 1, 0x27D4EB2D) ^ 0xB5297A4D) >>> 0
    );
    ...
    const alternateShuffleMap = alternateSource && alternateShuffleInput.checked
      ? makeTileShuffle(shuffleRandom, tileCount)
      : null;
  ...
  function renderFrame(frameIndex) {
    ...
    const dst = out.data;
    const {
      tile, tilesX, tileCount, addressFaults, dataFaults, stripeFaults,
      tileBanks, alternateShuffleMap,
      halfEvent, collapseEvent, pairEvent, collapseTile, collapseBank,
      colorGroupMask, colorAlternateEvent, colorPaletteEvent, colorTintEvent,
    } = plan;
    ...
    for (let y = 0; y < source.height; y++) {
      ...
      for (let x = 0; x < source.width; x++) {
        ...
        const colorForcesAlternate = colorMatch
          && alternateSource
          && (colorAlternateEvent || colorPaletteEvent);
        const readAlternate = !!alternateSource && (bank || colorForcesAlternate);
        const readData = readAlternate ? alternateSource.data : src;
        let readIndex = si;
        if (readAlternate && alternateShuffleMap) {
          const shuffledTile = alternateShuffleMap[sourceTile];
          const alternateX = Math.min(
            source.width - 1,
            (shuffledTile % tilesX) * tile + inX
          );
          const alternateY = Math.min(
            source.height - 1,
            Math.floor(shuffledTile / tilesX) * tile + sampleInY
          );
          readIndex = (alternateY * source.width + alternateX) * 4;
        }
        ...
        if (colorMatch && colorPaletteEvent) {
          const luminance = (
            readData[readIndex] * 54
            + readData[readIndex + 1] * 183
            + readData[readIndex + 2] * 19
          ) >> 8;
          const bucket = Math.min(3, luminance >> 6);
          ...
          dst[di] = readData[readIndex];
          dst[di + 1] = readData[readIndex + 1];
          dst[di + 2] = readData[readIndex + 2];
          dst[di + 3] = readData[readIndex + 3];
        }
  ...
  alternateShuffleInput.addEventListener('change', rerenderFromControl);
  ...
  function serializeFaultPlan(plan) {
    ...
    return {
      ...
      alternateShuffleMap: plan.alternateShuffleMap
        ? Array.from(plan.alternateShuffleMap)
        : null,
