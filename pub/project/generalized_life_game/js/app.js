...
import { 
  ...
  generateDanzer7FoldTiling,
  generateDanzer7FoldSubstitutionDraft,
  generateTetrahedron, 
  generateCube, 
  generateOctahedron, 
...
class App {
  ...
  rebuildGrid() {
    ...
    if (is3D) {
      ...
      try {
        ...
        this.cliConsole.log(`Ammann-Beenker generation error: ${err.message}. Retrying with projection range 3.`, 'error-line');
        this.complex.clear();
        generateAmmannBeenkerTiling(this.complex, 3);
      }
      if (this.complex.faces.length === 0) {
        this.cliConsole.log('Ammann-Beenker generated no faces. Retrying with projection range 3.', 'error-line');
        this.complex.clear();
        generateAmmannBeenkerTiling(this.complex, 3);
      }
    } else if (this.gridType === 'substitution-danzer-7fold') {
      generateDanzer7FoldTiling(this.complex, this.jitterCount);
    } else if (this.gridType === 'substitution-danzer-7fold-rule') {
      generateDanzer7FoldSubstitutionDraft(this.complex, this.jitterCount);
    ...
    if (this.gridType === 'substitution-ammann-beenker' && this.complex.faces.length === 0) {
      this.cliConsole.log('Ammann-Beenker patch was empty after cleanup. Rebuilding with projection range 3.', 'error-line');
      this.complex.clear();
      generateAmmannBeenkerTiling(this.complex, 3);
      this.complex.cleanAndReindex();
    }
  ...
  hasParameterSlider(type = this.gridType) {
    return [
      ...
      'substitution-danzer-7fold',
      'substitution-danzer-7fold-rule',
      'substitution-custom',
      'octa-chain',
      'prism',
      'octa-grid',
  ...
  isSubstitutionType(type = this.gridType) {
    return ['substitution-chair', 'substitution-fibonacci', 'substitution-penrose', 'substitution-penrose-rhomb', 'substitution-ammann-beenker', 'substitution-danzer-7fold', 'substitution-danzer-7fold-rule', 'substitution-custom'].includes(type);
  }
  ...
  getEvaluatedNeighborCount(aliveWeight, totalWeight) {
    return this.ruleEvaluator.getActiveRuleContext(aliveWeight, totalWeight).evaluatedCount;
  }
  ...
  getRatioRuleRows() {
    ...
    return Array.from(totals).sort((a, b) => a - b).map(totalWeight => {
      ...
      const override = this.ruleEvaluator.getRatioRuleOverride(totalWeight);
      return {
        totalWeight,
        birthCounts,
        survivalCounts,
        override,
        birthText: override ? this.ruleEvaluator.formatActualCountList(override.birthSet) : '',
        survivalText: override ? this.ruleEvaluator.formatActualCountList(override.survivalSet) : ''
      };
    });
  }

  applyRatioRuleRow(row) {
    if (!row) return;
    const totalWeight = Number(row.dataset.ratioTotal);
    if (!Number.isFinite(totalWeight)) return;
    const birthInput = row.querySelector('[data-ratio-kind="birth"]');
    const survivalInput = row.querySelector('[data-ratio-kind="survival"]');
    this.ruleEvaluator.setRatioRuleOverride(
      totalWeight,
      birthInput ? birthInput.value : '',
      survivalInput ? survivalInput.value : ''
    );
    this.updateRatioRuleMap();
    this.inspectLockedFace();
  }
  ...
  updateRatioRuleMap() {
    ...
    note.textContent = `Blank rows use round(alive / N * 8) from ${this.ruleEvaluator.ruleString}: B${birth} / S${survive}. Filled rows use actual alive counts for that N.`;
    ...
    table.innerHTML = rows.map(row => `
      <div class="ratio-rule-row ${row.override ? 'is-custom' : ''}" data-ratio-total="${row.totalWeight}">
        <span class="ratio-rule-degree">N=${row.totalWeight}</span>
        <label class="ratio-rule-field">
          <span>B</span>
          <input type="text" data-ratio-kind="birth" value="${this.escapeAttr(row.birthText)}" placeholder="${this.escapeAttr(this.formatActualCounts(row.birthCounts, row.totalWeight))}">
        </label>
        <label class="ratio-rule-field">
          <span>S</span>
          <input type="text" data-ratio-kind="survival" value="${this.escapeAttr(row.survivalText)}" placeholder="${this.escapeAttr(this.formatActualCounts(row.survivalCounts, row.totalWeight))}">
        </label>
        <button class="ratio-rule-reset" type="button" title="Use ratio-derived rule for this N">Reset</button>
      </div>
    `).join('');
  }
  ...
  configureParameterSlider(type, resetValue = false) {
    ...
    if (type === 'voronoi') {
      ...
      slider.min = 2; slider.max = 7; slider.step = 1;
      nextValue = 5;
    } else if (type === 'substitution-danzer-7fold') {
      label.textContent = "Danzer Iterations";
      slider.min = 0; slider.max = 6; slider.step = 1;
      nextValue = 3;
    } else if (type === 'substitution-danzer-7fold-rule') {
      label.textContent = "Danzer Rule Iterations";
  ...
  setupUIEvents() {
    ...
    document.getElementById('select-rule-preset').addEventListener('change', (e) => {
      ...
      if (presetName === 'custom-text') {
        ...
        if (preset) {
          ...
          this.ruleEvaluator.clearRatioRuleOverrides();
          this.updateUIFromState();
    ...
    document.getElementById('ratio-rule-table').addEventListener('change', (e) => {
      if (!e.target.matches('[data-ratio-kind]')) return;
      this.applyRatioRuleRow(e.target.closest('.ratio-rule-row'));
    });

    document.getElementById('ratio-rule-table').addEventListener('click', (e) => {
      const resetButton = e.target.closest('.ratio-rule-reset');
      if (!resetButton) return;
      const row = resetButton.closest('.ratio-rule-row');
      const totalWeight = Number(row?.dataset.ratioTotal);
      if (!Number.isFinite(totalWeight)) return;
      this.ruleEvaluator.clearRatioRuleOverride(totalWeight);
      this.updateRatioRuleMap();
      this.inspectLockedFace();
    });
  ...
  inspectFace(faceId) {
    ...
    if (this.ruleEvaluator.correctionMode === 'ratio' && totalWeight > 0 && this.ruleEvaluator.type !== 'custom-js') {
      const activeRule = this.ruleEvaluator.getActiveRuleContext(aliveWeight, totalWeight);
      const birthMatch = activeRule.birthSet.has(evaluatedCount);
      const survivalMatch = activeRule.survivalSet.has(evaluatedCount);
      ...
      const ruleLabel = activeRule.source === 'ratio-override' ? 'custom N rule' : 'round(*8)';
      document.getElementById('inspect-explanation').textContent =
        `Ratio: ${aliveLabel}/${totalLabel} -> ${ruleLabel} = ${evaluatedCount}. ` +
  ...
  serializeProjectData() {
    return {
      ...
      config: {
        ...
        ratioRuleOverrides: this.ruleEvaluator.serializeRatioRuleOverrides(),
  ...
  deserializeProjectData(data) {
    ...
    this.loopDetectionWindow = conf.loopDetectionWindow || 60;
    ...
    this.ruleEvaluator.deserializeRatioRuleOverrides(conf.ratioRuleOverrides || []);
