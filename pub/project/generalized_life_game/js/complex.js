...
// Target inverse inflation for Danzer's 7-fold rule. The draft generator below
// uses midpoint subdivision until the exact child-triangle coordinates are traced.
const DANZER_7_SCALE = Math.sin(Math.PI / 7) / (Math.sin(Math.PI / 7) + Math.sin(2 * Math.PI / 7));
...
function keepLargestConnectedTilePatch(tiles) {
  ...
  const adjacency = [];
  const edgeOwners = Object.create(null);
  ...
  Object.keys(edgeOwners).forEach(key => {
    const indices = edgeOwners[key];
    if (indices.length < 2) return;
    for (let i = 0; i < indices.length; i++) {
      for (let j = 0; j < indices.length; j++) {
        addAdjacency(indices[i], indices[j]);
      }
    }
  });
  ...
  for (let index = 0; index < tiles.length; index++) {
    ...
    while (stack.length) {
      ...
      for (let i = 0; i < adjacency[current].length; i++) {
        const next = adjacency[current][i];
        if (!seen[next]) {
          seen[next] = true;
          stack.push(next);
        }
  ...
  return tiles.filter((_, index) => keep[index]);
}
...
function generateProjection4DTiling(complex, definition, iterations = 5) {
  ...
  const seenTiles = Object.create(null);
  const tiles = [];
...
function triangleEdgeLengths(points) {
  return [
    Math.sqrt(distanceSq(points[0], points[1])),
    Math.sqrt(distanceSq(points[1], points[2])),
    Math.sqrt(distanceSq(points[2], points[0]))
  ].sort((a, b) => a - b);
}

function triangleCircumcircle(points, tri) {
...
function delaunayTriangulate(points) {
  ...
  const workPoints = points.concat([
  ...
  for (let pointIndex = 0; pointIndex < points.length; pointIndex++) {
    ...
    const edgeValues = Object.create(null);

    for (let i = 0; i < triangles.length; i++) {
      const tri = triangles[i];
      const circle = triangleCircumcircle(workPoints, tri);
      const isBad = circle && distanceSq(point, circle.center) <= circle.radiusSq + 1e-9;
      if (!isBad) {
    ...
    Object.keys(edgeCounts).forEach(key => {
      if (edgeCounts[key] === 1) {
        const edge = edgeValues[key];
        nextTriangles.push({ a: edge[0], b: edge[1], c: pointIndex });
      }
    });
    triangles = nextTriangles;
  }

  return triangles.filter(tri => tri.a < superStart && tri.b < superStart && tri.c < superStart);
}

function buildDanzer7FoldTriangles(iterations = 4) {
  const detail = Math.max(1, Math.min(6, parseInt(iterations, 10) || 4));
  const range = detail + 2;
  const clipRadius = range * 0.92;
  const normals = [];
  const offsets = [0.11, 0.27, 0.43, 0.58, 0.73, 0.89, 0.36];
  const pointMap = Object.create(null);
  const points = [];
  const minPointSpacingSq = 0.045 * 0.045;

  for (let i = 0; i < 7; i++) {
    normals.push(angleVector((180 * i) / 7));
  }

  const addPoint = (point) => {
    if ((point.x * point.x + point.y * point.y) > clipRadius * clipRadius) return;
    const key = pointKey(point);
    if (pointMap[key]) return;
    for (let i = 0; i < points.length; i++) {
      if (distanceSq(points[i], point) < minPointSpacingSq) return;
    }
    pointMap[key] = true;
    points.push({ x: roundCoord(point.x), y: roundCoord(point.y) });
  };
  ...
  const rawTriangles = delaunayTriangulate(points).map(tri => ({
    points: [points[tri.a], points[tri.b], points[tri.c]]
  })).filter(tile => {
    const centroid = tile.points.reduce((acc, point) => ({
      x: acc.x + point.x / 3,
      y: acc.y + point.y / 3
    }), { x: 0, y: 0 });
    return (centroid.x * centroid.x + centroid.y * centroid.y) <= clipRadius * clipRadius;
  });

  const tiles = rawTriangles.filter(tile => {
    const area = Math.abs(signedPolygonArea(tile.points));
    return area > 1e-5;
  }).map(tile => {
    let points = tile.points.map(point => ({ x: roundCoord(point.x), y: roundCoord(point.y) }));
    if (signedPolygonArea(points) < 0) points = points.reverse();
    const edges = triangleEdgeLengths(points);
    const ratio = edges[2] / Math.max(edges[0], 1e-9);
    const type = ratio < 1.28 ? 'A' : (ratio < 1.75 ? 'B' : 'C');
    return { type, points };
  });

  return keepLargestConnectedTilePatch(tiles);
}

export function generateDanzer7FoldTiling(complex, iterations = 4) {
  addPolygonTilingFaces(
    complex,
    buildDanzer7FoldTriangles(iterations),
    ['danzer-7fold-vertex', 'substitution-vertex'],
    ['danzer-7fold-triangle', 'substitution-tile']
  );
}

// The point order is the recursive parent basis for each child type, not just
// the visual polygon order from the traced PDF.
const DANZER_7_SUBSTITUTION_RULES = {
  A: [
    { type: 'A', order: [0, 2, 1], points: [[0.357389, 0.643414], [0, 1], [0, 0.643307]] },
    { type: 'A', order: [2, 0, 1], points: [[0.357389, 0.643414], [0.356135, 0.285954], [0, 0.643307]] },
    { type: 'B', order: [0, 2, 1], points: [[1, 0], [0.356135, 0.285954], [0, 0]] },
    { type: 'C', order: [2, 0, 1], points: [[1, 0], [0.356135, 0.285954], [0.357389, 0.643414]] },
    { type: 'C', order: [2, 0, 1], points: [[0, 0], [0.356135, 0.285954], [0, 0.643307]] }
  ],
  B: [
    { type: 'A', order: [0, 2, 1], points: [[0.286329, 0.197997], [0.642807, 0], [0.356411, 0]] },
    { type: 'B', order: [2, 0, 1], points: [[0.642807, 0], [0.642807, 0.356749], [1, 0]] },
    { type: 'B', order: [0, 2, 1], points: [[0.642807, 0], [0.642807, 0.356749], [0.286329, 0.555123]] },
    { type: 'B', order: [2, 0, 1], points: [[0.642807, 0], [0.286329, 0.197997], [0.286329, 0.555123]] },
    { type: 'B', order: [0, 2, 1], points: [[0, 0], [0, 0.357281], [0.356411, 0]] },
    { type: 'B', order: [0, 2, 1], points: [[0.356411, 0], [0, 0.357281], [0.158595, 0.555279]] },
    { type: 'C', order: [0, 2, 1], points: [[0, 1], [0.286329, 0.555123], [0.642807, 0.356749]] },
    { type: 'C', order: [2, 0, 1], points: [[0, 1], [0.286329, 0.555123], [0.286329, 0.197997]] },
    { type: 'C', order: [0, 2, 1], points: [[0, 1], [0.158595, 0.555279], [0, 0.357281]] }
  ],
  C: [
    { type: 'A', order: [2, 0, 1], points: [[0.801242, 0.001252], [1, 0], [0.643125, 0.357278]] },
    { type: 'A', order: [0, 2, 1], points: [[0.801242, 0.001252], [0.444433, 0.357467], [0.643125, 0.357278]] },
    { type: 'B', order: [0, 2, 1], points: [[0.444433, 0.001053], [0.444433, 0.357467], [0, 1]] },
    { type: 'B', order: [2, 0, 1], points: [[0.444433, 0.001053], [0, 0.642824], [0, 1]] },
    { type: 'B', order: [2, 0, 1], points: [[0.444433, 0.001053], [0, 0.642824], [0.197783, 0]] },
    { type: 'C', order: [2, 0, 1], points: [[0.444433, 0.001053], [0.444433, 0.357467], [0.801242, 0.001252]] },
    { type: 'C', order: [2, 0, 1], points: [[0, 1], [0.444433, 0.357467], [0.643125, 0.357278]] },
    { type: 'C', order: [0, 2, 1], points: [[0, 0], [0.197783, 0], [0, 0.642824]] }
  ]
};

function snapDanzerRuleCoord(value) {
  if (Math.abs(value) < 0.002) return 0;
  if (Math.abs(value - 1) < 0.002) return 1;
  return value;
}

function mapDanzerRulePoint(parentPoints, localPoint) {
  const [a, b, c] = parentPoints;
  const u = snapDanzerRuleCoord(localPoint[0]);
  const v = snapDanzerRuleCoord(localPoint[1]);
  return {
...
function substituteDanzerRuleTile(tile) {
  const rule = DANZER_7_SUBSTITUTION_RULES[tile.type] || DANZER_7_SUBSTITUTION_RULES.A;
  return rule.map(child => {
    const mappedPoints = child.points.map(point => mapDanzerRulePoint(tile.points, point));
    const order = child.order || [0, 1, 2];
    return {
      type: child.type,
      points: order.map(index => mappedPoints[index])
    };
  });
}

function buildDanzer7FoldSubstitutionDraft(iterations = 4) {
  const maxIterations = Math.max(0, Math.min(3, parseInt(iterations, 10) || 0));
  const radius = 1;
  let tiles = [];

  for (let i = 0; i < 14; i++) {
    const a0 = (2 * Math.PI * i) / 14;
    const a1 = (2 * Math.PI * (i + 1)) / 14;
    tiles.push({
      type: 'A',
      points: [
        { x: radius * Math.cos(a0), y: radius * Math.sin(a0) },
  ...
  for (let i = 0; i < maxIterations; i++) {
    ...
    for (let j = 0; j < tiles.length; j++) {
      next.push(...substituteDanzerRuleTile(tiles[j]));
    }
    tiles = next;
  }
...
export function generateDanzer7FoldSubstitutionDraft(complex, iterations = 4) {
  addPolygonTilingFaces(
    complex,
    buildDanzer7FoldSubstitutionDraft(iterations),
    ['danzer-7fold-rule-vertex', 'substitution-vertex'],
    ['danzer-7fold-rule-triangle', 'substitution-tile']
  );
}
