...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function playYoutubeVideo(url, saveHistory = true) {
        ...
        if (!videoId || !placeholder || !wrapper) {
            window.say('それは有効なYouTubeのURLじゃないかも……？', 'angry');
            return;
        }

        // file:// プロトコル(ローカル起動)でのYouTube再生バグ回避
        // file:// の場合は youtube-nocookie.com を使用し、origin パラメータおよび enablejsapi を付与しない（後者があるとpostMessageでエラーになるため）
        // かつ autoplay ポリシーのために mute=1 を付与
        const isLocalFile = window.location.protocol === 'file:';
        const domain = isLocalFile ? 'https://www.youtube-nocookie.com' : 'https://www.youtube.com';
        let embedUrl = `${domain}/embed/${videoId}?autoplay=1&mute=1`;
        
        if (!isLocalFile) {
            // 通常サーバー環境ではJS APIを有効化し、オリジンを明示して安全に連携
        ...
        window.say('動画を開くね。作業用BGMかな？', 'present');
    ...
    function handlePlayCommand(subArgs) {
        const url = subArgs[0];
        if (!url) {
            printCli('エラー: 再生するYouTube URLを指定してください。', 'danger');
            return;
        }
        switchTab('video');
        document.getElementById('videoUrlInput').value = url;
        playYoutubeVideo(url);
        printCli(`動画の埋め込み再生を開始しました: ${url}`, 'success');
    }
    ...
    function getVfs() {
        let raw = localStorage.getItem(VFS_KEY);
        let vfs = { files: {}, dirs: ['/'] };
        if (raw) {
            try {
                ...
                if (parsed && parsed.files && parsed.dirs) {
                    ...
                    // Migrate flat VFS to hierarchical VFS
